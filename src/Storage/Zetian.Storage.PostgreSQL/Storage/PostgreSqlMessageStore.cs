    ...
    public class PostgreSqlMessageStore : IMessageStore, IDisposable
    {
        ...
        public async Task<bool> SaveAsync(ISmtpSession session, ISmtpMessage message, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                command.Parameters.AddWithValue("@from_address", (object?)message.From?.Address ?? DBNull.Value);
        ...
        private async Task EnsureTableExistsAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!exists)
                {
                    ...
                    {
                        await using NpgsqlCommand partitionCommand = connection.CreateCommand();
                        partitionCommand.CommandText = $@"
                            -- Convert to partitioned table by date range
                            CREATE TABLE {_configuration.GetFullTableName()}_partitioned 
                            (LIKE {_configuration.GetFullTableName()} INCLUDING ALL)
                            PARTITION BY RANGE (received_date)";

                        // Add logic to create monthly partitions
                        _logger?.LogInformation("Table partitioning enabled for {TableName}", _configuration.GetFullTableName());
                    }
                    ...
                    _logger?.LogInformation("Created PostgreSQL table {TableName}", _configuration.GetFullTableName());
                }
